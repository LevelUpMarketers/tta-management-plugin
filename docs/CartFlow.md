...
3. **Checkout**
   ...
   - Countdown timers run just like on the cart page. As soon as the user begins filling out the form an AJAX request freezes the timers and stores the remaining time for each ticket.
   - If payment fails the cart unlocks and `TTA_Cart::resume_items()` restores each expiration using the saved remaining time.
