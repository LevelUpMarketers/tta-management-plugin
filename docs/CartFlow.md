...
3. **Checkout**
  ...
  - When a membership is present, the server first sends a test `ARBCreateSubscriptionRequest` using the payment token to detect existing subscriptions. If the probe succeeds, the token is consumed so the browser is asked to re-tokenize before the real charge proceeds. A duplicate (`E00012`) aborts checkout with an error and no charge is made. Any other failure shows a generic retry message. Carts without memberships skip this probe.
