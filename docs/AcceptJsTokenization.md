...
- Member-dashboard updates now refresh the Customer Information Manager (CIM) payment profile with the Accept.js token before reattaching it to the ARB subscription. If an ARB subscription does not have a profile yet, the handler creates one with the token so no OTS token is sent directly to ARB. This avoids reusing an OTS token directly against the subscription and keeps the payment profile and billing address in sync.
