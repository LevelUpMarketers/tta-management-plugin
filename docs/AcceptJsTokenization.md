...
- Member-dashboard updates now refresh the existing Customer Information Manager (CIM) payment profile linked to the ARB subscription using the Accept.js token. The subscription details provide the customerProfileId and customerPaymentProfileId at runtime, and the handler updates that payment profile directly instead of sending the opaque token to ARBUpdateSubscription.
