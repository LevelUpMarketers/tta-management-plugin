...
- **Manage Partners:** Lists saved partners in an expandable table. Clicking a row opens an inline form pre-populated with the partner's details (including IDs for the autogenerated admin/login pages). The form saves changes via the standard spinner/response UI and syncs the contact info back to the WordPress Subscriber account.
